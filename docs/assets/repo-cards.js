window.addEventListener('DOMContentLoaded', async function () {
    ...
    function setCard(el, data, theme) {
        const repoAttr = el.getAttribute('data-repo') || '';
        // GitHub API 限流(未认证 60 次/小时)或出错时,响应 body 没有 name 字段,
        // 用 data-repo 属性兜底渲染,避免出现 "No name available"
        if (!data || typeof data.name !== 'string') {
            data = {
                name: repoAttr,
                description: '仓库信息暂不可用（GitHub API 限流或网络波动），刷新后自动恢复',
                html_url: repoAttr ? 'https://github.com/' + repoAttr : '#',
                language: null,
        ...
        const name = data.name || repoAttr || 'No name available';
        const description = data.description || 'No description available';
        const html_url = data.html_url || (repoAttr ? 'https://github.com/' + repoAttr : '#');
        const language = data.language || 'Unknown';
        const stargazers_count = data.stargazers_count !== undefined ? data.stargazers_count : null;
        const forks = data.forks !== undefined ? data.forks : null;
        const forked_from = data.fork ? ((data.source && data.source.full_name) || 'Unknown source') : '';
        ...
        el.innerHTML = `
            <div style="flex-direction: column; height: 100%; display: flex;
              ...
              <div style="display: ${data.fork ? 'block' : 'none'}; font-size: 12px; color: ${theme.color};">Forked from <a style="color: inherit; text-decoration: none;" href="${data.fork ? data.source.html_url : '#'}">${forked_from}</a></div>
              <div style="font-size: 12px; margin-bottom: 16px; margin-top: 8px; color: ${theme.color}; flex: 1;">${description}</div>
              <div style="font-size: 12px; color: ${theme.color}; display: flex; flex: 0;">
                ...
                <div style="display: ${(stargazers_count === 0 || stargazers_count == null) ? 'none' : 'flex'}; align-items: center; margin-right: 16px;">
                ...
                <div style="display: ${(forks === 0 || forks == null) ? 'none' : 'flex'}; align-items: center;">
    ...
    function promiseGet(url, retries = 3) {
        return new Promise((resolve, reject) => {
            ...
            if (prevResp && Math.abs(now - prevResp.time) < 60000 * 30) {
                ...
                (async function fetchWithRetries(attempt = 1) {
                    try {
                        const resp = await fetch(url);
                        if (!resp.ok) throw new Error('HTTP error! status: ' + resp.status);
                        const json = await resp.json();
    ...
    for (const el of document.querySelectorAll('.repo-card')) {
        ...
        promiseGet(url).then(data => {
            ...
            // 优先用过期缓存兜底,其次用 data-repo 属性渲染占位卡片
            let stale = null;
            try { stale = JSON.parse(localStorage.getItem(url)); } catch (e) {}
            if (stale && stale.data) {
                setCard(el, stale.data, theme);
            } else {
                setCard(el, null, theme);
            }
        });
    }
});
