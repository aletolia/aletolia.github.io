...
> (E) 二维图显示了沿着推断的伪时间，路径 1（红色）和路径 2（蓝色）的细胞中高亲和力、低亲和力、耗竭和 CSR 签名的动态表达得分。中心线表示线性拟合，阴影线表示 95% 置信区间。
...
> (P) 根据 Bm 和 AtM B 细胞的签名得分，分层治疗的皮肤黑色素瘤（SKCM）患者（n = 26）的反应率（左）和总生存（右）的情况。CR，完全缓解；PR，部分缓解；PD，进展性疾病；SD，稳定疾病。
> (Q) 根据 Bm 和 AtM B 细胞的签名得分分层的肺癌（LC）患者（n = 26）接受抗 PD1 治疗的无进展生存 Kaplan-Meier 曲线。数据显示中位数和 IQR 在 (C) 和 (M) 中；在 (J)，(L) 和 (N) 中显示最小和最大测量值。\*P < 0.05，\*\*P < 0.01，\*\*\*P < 0.001，\*\*\*\*P < 0.0001；ns, 无显著性。P 值由双侧 Wilcoxon 检验确定 (C)，(D) 和 (F)；Mann-Whitney 检验确定 (D) 和 (N)；双侧配对学生 t 检验确定 (L) 至 (N)；双侧对数秩检验确定 (O)（右侧），(P) 和 (Q)，以及 χ² 检验确定 (P)（左侧）。
